...
serve(async (req) => {
  ...
  try {
    ...
    if (data_inicio_raw && !data_inicio) {
      return new Response(JSON.stringify({ ok: false, error: "data_inicio inválida. Use YYYY-MM-DD." }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 400,
      });
    }
    if (data_fim_raw && !data_fim) {
      return new Response(JSON.stringify({ ok: false, error: "data_fim inválida. Use YYYY-MM-DD." }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 400,
      });
    }

    const { data, error } = await supabaseClient.rpc("calculate_media_intervalo_partos", {
    ...
    const result = (data ?? [])[0] ?? { valor_medio: 0, total_matrizes: 0 };
