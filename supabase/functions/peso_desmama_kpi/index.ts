...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      sexo = url.searchParams.get("sexo"); // 'M' | 'F' | 'T' | null
    } else {
    ...
    const { data, error } = await supabase.rpc("peso_desmama_medio", {
      p_inicio: inicio,
      p_fim: fim,
    ...
    // A função SQL retorna: { sexo: string, valor: number, n: number }[]
    // O widget espera: { valor: number, titulo?: string, sufixo?: string }
    // Se houver múltiplos resultados (por sexo), pega o primeiro ou agrupa
    const items = Array.isArray(data) && data.length > 0
      ? [{
          valor: Number(data[0].valor ?? 0),
          titulo: "",
          sufixo: " kg",
          total_animais: Number(data[0].n ?? 0),
        }]
      : [{
          valor: 0,
          titulo: "",
