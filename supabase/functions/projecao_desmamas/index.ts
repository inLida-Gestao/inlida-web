// ==================================================
// EDGE FUNCTION COMPLETA ATUALIZADA: desmama_periodo
// ==================================================
// - Parsing de data robusto (aceita YYYY-M-D e YYYY-MM-DD) sem bug de fuso
// - idPropriedade obrigatório
// - sexo: "Todos" / "T" / vazio => sem filtro
// - agrupar=mes => chama projecao_desmamas
// - agrupar=bucket => chama desmamas_por_periodo_bucket (retorno direto)
...
serve(async (req) => {
  ...
  try {
    ...
    if (isMes) {
      const { data, error } = await supabase.rpc("projecao_desmamas", {
        p_inicio: inicio,
        p_fim: fim,
