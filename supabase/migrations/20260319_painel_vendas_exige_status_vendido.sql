-- Painel > Vendas: contar apenas animais efetivamente vendidos (status),
-- não só quem tem dataVenda preenchida. Evita gráficos de venda quando
-- status ainda é "Na propriedade" (dados inconsistentes ou fluxos antigos).
...
base as (
  ...
  where r.deletado = 'NAO'
    ...
    and lower(btrim(coalesce(r.status, ''))) = 'vendido'
