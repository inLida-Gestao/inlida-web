-- Taxa de concepção (painel): total_prenhe e total_inseminadas por categoria.
-- Ver 20260324140000_fix_calcular_taxa_prenhez_join_e_categoria.sql para correção de JOIN/categoria.
...
WITH raw AS (
  ...
  WHERE rep.id_propriedade = id_propriedade_param
    ...
    AND coalesce(rep.data_inseminacao, rep.data_inicial)::date >= data_inicio_param::date
    AND coalesce(rep.data_inseminacao, rep.data_inicial)::date <= data_fim_param::date
    AND (
...
agg AS (
  SELECT
    ...
    count(*) FILTER (WHERE b.status_reproducao = 'Prenhez')::bigint AS total_prenhe
  FROM base b
...
COMMENT ON FUNCTION public.calcular_taxa_prenhez(text, text, text, text, text, text) IS
  'Taxa de concepção por categoria (reproducao.categoria + fallback rebanho): prenhes / inseminadas no período.';
