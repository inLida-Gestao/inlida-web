DO $$
DECLARE
  def text;
BEGIN
  SELECT pg_get_functiondef('public.get_relatorio_resumo_estacao(text,text,text)'::regprocedure)
    INTO def;

  def := replace(
    def,
    'prenhez_nati_cond AS (
  SELECT s.*
  FROM serv s
  WHERE
    (',
    'prenhez_nati_cond AS (
  SELECT s.*
  FROM serv s
  WHERE s.id_rebanho_matriz IS NOT NULL
    AND
    ('
  );

  def := replace(
    def,
    'FROM serv n
  WHERE
    (',
    'FROM serv n
  WHERE n.id_rebanho_matriz IS NOT NULL
    AND ('
  );

  def := replace(
    def,
    'FROM serv n
  WHERE lower',
    'FROM serv n
  WHERE n.id_rebanho_matriz IS NOT NULL
    AND lower'
  );

  def := replace(
    def,
    'FROM serv s
  GROUP BY s.cat_titulo',
    'FROM serv s
  WHERE s.id_rebanho_matriz IS NOT NULL
  GROUP BY s.cat_titulo'
  );

  EXECUTE def;
END $$;

COMMENT ON FUNCTION public.get_relatorio_resumo_estacao(text, text, text) IS
  'Resumo estação: reproduções e diagnósticos nos mesmos registros com serviço no período (inseminação/início), incluindo linha Outras e considerando somente registros com matriz vinculada. Última DG: max(data_status) no período.';
