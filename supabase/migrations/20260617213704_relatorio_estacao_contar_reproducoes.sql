-- Relatório da estação: conta registros de reprodução no período, não matrizes
-- distintas. Mantém diagnósticos na mesma base — registros com serviço
-- (COALESCE(inseminação, início)) no período.
--
