-- Relatório da estação: Natimorto deixa de contar como PRENHEZ e passa a contar
-- em OUTROS (junto de aborto/absorção/não diagnosticado).
--
-- Bug (WEB - P.ALTA): reproduções com diagnóstico "Natimorto" eram somadas na
-- coluna PRENHEZ (e no % de prenhez, prenhas por IA e por touro), inflando a
-- prenhez positiva. Natimorto é desfecho de perda — coerente com os demais
-- desfechos, vai para OUTROS.
--
-- Base: 20260617213704 (conta registros de reprodução) + 20260617214637
-- (inclui reproduções sem matriz — mantido: os CTEs não filtram mais por
-- id_rebanho_matriz). Aqui só muda a classificação de natimorto.
...
-- Prenhez positiva = status prenhe/prenha. Natimorto NÃO entra aqui (vai p/ outros).
prenhez_cond AS (
  SELECT s.*
  FROM serv s
  WHERE
...
vazio_dist AS (
  SELECT
    ...
    count(*)::bigint AS n
  FROM serv n
  WHERE lower(trim(coalesce(n.status_reproducao, ''))) = 'vazio'
  GROUP BY n.cat_titulo
),
outros_dist AS (
  SELECT
    ...
    count(*)::bigint AS n
  FROM serv n
  WHERE lower(trim(coalesce(n.status_reproducao, ''))) IN (
    'absorção',
    'absorcao',
    'aborto',
    'natimorto',
...
r_mult AS (
  ...
  LEFT JOIN prenhez_reg na ON na.cat_titulo = 'Vaca Multipara'
...
r_prim AS (
  ...
  LEFT JOIN prenhez_reg na ON na.cat_titulo = 'Vaca Primipara'
...
r_nov AS (
  ...
  LEFT JOIN prenhez_reg na ON na.cat_titulo = 'Novilha'
...
r_outras AS (
  ...
  LEFT JOIN prenhez_reg na ON na.cat_titulo = 'Outras'
...
r_total AS (
  SELECT
    ...
    coalesce(
      round(100.0 * (SELECT prt.n FROM prenhez_reg_total prt)::numeric / nullif(et.n, 0), 2),
      0
    )::numeric AS prenhez_pct
  FROM
    prenhez_total p
...
COMMENT ON FUNCTION public.get_relatorio_resumo_estacao(text, text, text) IS
  'Resumo estação: reproduções com serviço no período (inseminação/início), incluindo registros sem matriz e linha Outras. Prenhez = status prenhe/prenha; natimorto/aborto/absorção/não diagnosticado contam em Outros. Última DG: max(data_status) no período.';
