-- Ordena a listagem de lotes no banco antes da paginacao.
--
-- A UI usa paginacao manual; ordenar somente a pagina atual deixa o conjunto
-- completo em id DESC. Estes parametros opcionais preservam chamadas antigas.
...
AS $function$
  ...
  ids_no_array AS (
    SELECT lf.id AS lote_pk, jsonb_array_elements_text(lf.id_animais::jsonb) AS reb_id_str
    FROM lote_filtrado lf
    WHERE lf.id_animais IS NOT NULL
      AND btrim(lf.id_animais) <> ''
      AND lower(btrim(lf.id_animais)) NOT IN ('null', '[]')
      AND left(btrim(lf.id_animais), 1) = '['
  ),
  match_id_animais AS (
