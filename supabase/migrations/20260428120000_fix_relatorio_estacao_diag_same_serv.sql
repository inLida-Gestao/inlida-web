-- Relatório da estação: diagnósticos (Prenhez/Vazio/Outros/IA/touro) na mesma base
-- que "matrizes" — registros com serviço (COALESCE(inseminação, início)) no período.
--
...
COMMENT ON FUNCTION public.get_relatorio_resumo_estacao(text, text, text) IS
  'Resumo estação: matrizes e diagnósticos nos mesmos registros com serviço no período (inseminação/início). Última DG: max(data_status) no período.';
