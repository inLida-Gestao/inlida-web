-- Corrige a taxa de natalidade para não contar partos de outros ciclos.
-- Numerador: matrizes da base do período com parida confirmada no próprio
-- registro reprodutivo do período. Assim a natalidade não ultrapassa prenhez.
...
COMMENT ON FUNCTION public.calcular_taxa_natalidade(text, text, text, text, text, text) IS
  'Taxa de natalidade (%): denominador = matrizes com inseminação ou monta natural no período selecionado; numerador = dessas matrizes, as marcadas como parida SIM no próprio registro reprodutivo do período, excluindo natimortos.';
