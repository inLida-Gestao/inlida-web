-- Corrige o numerador da taxa de natalidade: depois de encontrar as
-- matrizes expostas no período, procura parto confirmado em qualquer
-- registro reprodutivo da mesma matriz na propriedade.
...
expostas AS (
  ...
  FROM raw_activity r
  GROUP BY r.id_rebanho_matriz
),
partos_por_matriz AS (
  SELECT
    rep.id_rebanho_matriz,
    bool_or(
      (
        rep.data_parto IS NOT NULL
        OR upper(trim(coalesce(rep.parida, ''))) IN ('SIM', 'S', 'TRUE', '1')
      )
      AND lower(trim(coalesce(rep.status_reproducao, ''))) NOT LIKE '%natimorto%'
    ) AS pariu
  FROM public.reproducao rep
  JOIN expostas e
    ON e.id_rebanho_matriz = rep.id_rebanho_matriz
  WHERE rep.id_propriedade = id_propriedade_param
    AND (rep.deletado IS NULL OR rep.deletado = 'NAO')
    AND (rep.ressinc IS NULL OR rep.ressinc <> 'SIM')
  GROUP BY rep.id_rebanho_matriz
),
agg AS (
  SELECT
    e.cat_titulo AS titulo,
    count(*)::bigint AS total_expostas,
    count(*) FILTER (WHERE coalesce(p.pariu, false))::bigint AS total_pariram
  FROM expostas e
  LEFT JOIN partos_por_matriz p
    ON p.id_rebanho_matriz = e.id_rebanho_matriz
  GROUP BY e.cat_titulo
)
SELECT
  a.titulo,
  CASE
...
COMMENT ON FUNCTION public.calcular_taxa_natalidade(text, text, text, text, text, text) IS
  'Taxa de natalidade (%): denominador = matrizes com inseminação ou monta natural no período selecionado; numerador = dessas matrizes, as com data_parto preenchida ou parida confirmada em qualquer registro reprodutivo da mesma matriz, excluindo natimortos.';
