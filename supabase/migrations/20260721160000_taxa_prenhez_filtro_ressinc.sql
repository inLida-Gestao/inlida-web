-- Gráfico de concepção (calcular_taxa_prenhez): filtro por tipo de reprodução,
-- em especial RESSINC.
--
-- Melhoria (WEB - P.BAIXA): permitir ver só as inseminações do período marcadas
-- como ressinc. Ressinc = protocolo cadastrado no registro (Tradicional /
-- Precoce / Superprecoce). '-', 'NAO', '' e 'null' NÃO são ressinc
-- (ver reproducao_d_t_struct: "[NAO] = sem protocolo").
--
-- Novo parâmetro p_tipo_reproducao:
--   ''/'todos'        -> sem filtro (comportamento atual)
--   'ressinc'         -> ressinc IN (tradicional, precoce, superprecoce)
--   'tradicional' | 'precoce' | 'superprecoce' -> aquele protocolo específico
-- O gráfico já é restrito a inseminações (data_inseminacao NOT NULL).
