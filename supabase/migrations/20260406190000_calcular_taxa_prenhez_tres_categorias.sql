-- Taxa de concepção (painel): apenas Novilha, Vaca Primipara, Vaca Multipara
-- (denominador = registros com data_inseminacao no período; numerador = status prenhe*).
...
COMMENT ON FUNCTION public.calcular_taxa_prenhez(text, text, text, text, text, text) IS
  'Taxa de concepção: prenhes (status prenhe*) / registros de inseminação (data_inseminacao no período), por categoria.';
