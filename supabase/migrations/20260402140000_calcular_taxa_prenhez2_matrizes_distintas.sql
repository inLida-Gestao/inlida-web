-- Taxa de concepção v2 (painel): denominador = matrizes distintas com data_inseminacao no período;
-- numerador = dessas matrizes, as que têm ao menos um registro com status_reproducao LIKE 'prenhe%'.
...
COMMENT ON FUNCTION public.calcular_taxa_prenhez2(text, text, text, text, text, text) IS
  'Taxa de concepção por categoria: matrizes prenhes (status prenhe*) / matrizes distintas expostas (com data_inseminacao no período).';
