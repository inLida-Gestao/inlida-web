-- Refina a taxa de natalidade:
-- base = matrizes inseminadas ou em monta natural no período selecionado;
-- numerador = matrizes dessa base com parida confirmada e data_parto no período.
...
COMMENT ON FUNCTION public.calcular_taxa_natalidade(text, text, text, text, text, text) IS
  'Taxa de natalidade (%): denominador = matrizes com inseminação ou monta natural no período selecionado; numerador = dessas matrizes, as com parida SIM e data_parto no período selecionado, excluindo natimortos.';
