-- Bug #1: Correção da taxa de natalidade.
-- Denominador: vacas com atividade reprodutiva no período
--   (data_inseminacao OU data_inicial/data_final dentro do intervalo).
-- Numerador: dessas vacas expostas, as que têm data_parto preenchida
--   nos MESMOS registros reprodutivos do período, SEM filtrar data_parto
--   pelo período. Exclui natimortos.
...
-- Registros reprodutivos no período (inseminação ou monta natural)
raw_activity AS (
  SELECT
  ...
  WHERE rep.id_propriedade = id_propriedade_param
    ...
    -- Atividade reprodutiva no período:
    -- inseminação dentro do período OU monta natural (data_inicial E data_final dentro do período)
    AND (
      (
...
base_activity AS (
  SELECT
    r.id_rebanho_matriz,
    -- Marca se este registro tem parto (SEM filtro de período na data_parto)
    (
...
-- Uma linha por vaca: ela pariu se QUALQUER registro dela no período tem parto
per_matriz AS (
  SELECT
    b.id_rebanho_matriz,
...
COMMENT ON FUNCTION public.calcular_taxa_natalidade(text, text, text, text, text, text) IS
  'Taxa de natalidade (%): denominador = vacas com atividade reprodutiva no período (inseminação OU monta com data_inicial+data_final no intervalo); numerador = dessas vacas, as com data_parto preenchida no registro reprodutivo do período (sem filtrar data_parto pelo período), excluindo natimortos.';
