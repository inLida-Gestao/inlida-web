-- Taxa de natalidade por ciclo reprodutivo:
-- base = matrizes inseminadas ou em monta natural no período selecionado;
-- numerador = matrizes dessa base marcadas como parida SIM.
-- A data_parto pode cair no ano seguinte ao ciclo reprodutivo.
...
COMMENT ON FUNCTION public.calcular_taxa_natalidade(text, text, text, text, text, text) IS
  'Taxa de natalidade (%): denominador = matrizes com inseminação ou data inicial de monta natural no período; numerador = dessas matrizes, as marcadas como parida SIM no ciclo reprodutivo, excluindo natimortos.';
