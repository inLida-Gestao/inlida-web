-- Paginate piquete animal/lote selectors so large properties do not load every
-- option into the client at once.

CREATE INDEX IF NOT EXISTS rebanho_piquete_options_idx
  ON public.rebanho ("idPropriedade", "numeroAnimal", nome)
  WHERE "idRebanho" IS NOT NULL
    AND COALESCE(deletado, 'NAO') = 'NAO';

CREATE INDEX IF NOT EXISTS rebanho_piquete_lote_count_idx
  ON public.rebanho ("idPropriedade", "loteID")
  WHERE COALESCE(deletado, 'NAO') = 'NAO';

CREATE INDEX IF NOT EXISTS rebanho_piquete_filter_idx
  ON public.rebanho (
    "idPropriedade",
    status,
    sexo,
...
CREATE INDEX IF NOT EXISTS lotes_piquete_options_idx
  ON public.lotes (id_propriedade, nome)
  WHERE id_lote IS NOT NULL
    AND COALESCE(deletado, 'NAO') = 'NAO';

CREATE INDEX IF NOT EXISTS lotes_piquete_filter_idx
  ON public.lotes (id_propriedade, ativo, created_at)
  WHERE id_lote IS NOT NULL
    AND COALESCE(deletado, 'NAO') = 'NAO';
...
CREATE OR REPLACE FUNCTION public.buscar_animais_piquete_por_ids(
  p_id_propriedade text DEFAULT '',
...
AS $$
DECLARE
  v_ids text[] := COALESCE(p_animais_ids, ARRAY[]::text[]);
  v_result jsonb;
BEGIN
  IF NOT public.usuario_tem_acesso_propriedade(p_id_propriedade) THEN
    RAISE EXCEPTION 'Usuario sem acesso a propriedade';
  ...
  SELECT COALESCE(jsonb_agg(item ORDER BY sort_index, numero, nome, id), '[]'::jsonb)
  INTO v_result
  FROM (
    SELECT
      array_position(v_ids, r."idRebanho") AS sort_index,
      COALESCE(r."numeroAnimal", '') AS numero,
      COALESCE(r.nome, '') AS nome,
      r."idRebanho" AS id,
      jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION public.buscar_animais_piquete_por_ids(text, text[]) TO authenticated;
