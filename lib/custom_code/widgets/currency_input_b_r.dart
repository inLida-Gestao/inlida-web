...
class CurrencyInputBR extends StatefulWidget {
  const CurrencyInputBR(
      {super.key,
      this.width,
      this.height,
      this.initialValue,
...
class _CurrencyInputBRState extends State<CurrencyInputBR> {
  ...
  void _updateAppState(double value) {
    ...
    if (value > 0) {
      FFAppState().valueDouble2 = value;
      FFAppState().update(() {});
    } else {
      // Se valor for zero ou negativo, limpa o App State
      FFAppState().valueDouble2 = 0.0;
      FFAppState().update(() {});
    }
  }
